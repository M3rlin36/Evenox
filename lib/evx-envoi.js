(function (global) {
  ...
  */
  ...
  function versFormData(champs) {
    ...
    if (champs.evx_test) fd.append('evx_test', champs.evx_test);
    return fd;
  }
  ...
  function mailtoHref(champs) {
    var sep = String.fromCharCode(38);
    var sujet = encodeURIComponent('[Evenox] Soumission (repli)');
    var details = '';
    if (champs) details = champs.details || '';
    var corps = encodeURIComponent(details);
    return 'mailto:evenox.ca@gmail.com?subject=' + sujet + sep + 'body=' + corps;
  }

  function mailtoRepli(champs) {
    var href = mailtoHref(champs);
    if (!global.__evxMailto) global.__evxMailto = [];
    global.__evxMailto.push(href);
    if (global.evx_test) return;
    try {
  ...
  function envoyer(opts) {
    ...
    if (global.evx_test) {
      champs.marqueur = 'TEST';
      champs.evx_test = 'TEST';
      if (champs.details) {
        if (champs.details.indexOf('TEST') !== 0) {
          champs.details = 'TEST' + String.fromCharCode(10) + champs.details;
        }
      }
    }
    poster(champs)
  ...
  var api = {
    ...
    hashLead: hashLead,
    lireFile: lireFile,
    ecrireFile: ecrireFile,
    mailtoHref: mailtoHref,
    CLE_FILE: CLE_FILE,
    CLE: CLE_FILE,
    MAX: MAX_TENTATIVES
  };
  api.MAX = MAX_TENTATIVES;
  api.ecrireFile = ecrireFile;
  global.EvxEnvoi = api;
  global.evxEnvoi = api;
