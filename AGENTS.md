...
- Étiqueter `GROS-File` **avant** de dire qu’on s’en occupe.
- 1 dossier à la fois (`GROS-En-cours`). Max 1 brouillon par run.
- Ne jamais scanner `is:unread` (20k+ fils). Utiliser `label:GROS-File`.
- Ne pas lancer un agent Cloud en boucle sur l’inbox — trop cher.
