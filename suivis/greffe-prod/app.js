...
(function () {
  ...
  function demarrer() {
    ...
    var feuillePlus = document.getElementById('plus-sheet');
    function fermerPlus() { if (feuillePlus) feuillePlus.classList.remove('on'); }

    document.querySelectorAll('.nav-i[data-vue]').forEach(function (b) {
      b.addEventListener('click', function () { aller(b.dataset.vue); fermerPlus(); });
    });

    // Onglet Plus (mobile) : ouvre la feuille Sequence / Closing / Clients.
    var btnPlus = document.getElementById('btn-plus');
    if (btnPlus && feuillePlus) {
      btnPlus.addEventListener('click', function (e) { e.stopPropagation(); feuillePlus.classList.toggle('on'); });
      feuillePlus.addEventListener('click', function (e) { if (e.target === feuillePlus) fermerPlus(); });
    }
