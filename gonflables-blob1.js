(function(){
  ...
  var PRODUITS = [
    { id:'jeux-gonflable-moyen-bouncer', nom:'Moyen Bouncer', prix:120, groupe:'petits' },
    { id:'jeux-gonflable-forteresse', nom:'Forteresse', prix:100, groupe:'petits' },
    { id:'jeu-gonflable-mini-princesse', nom:'Princesses', prix:120, groupe:'petits' },
    { id:'jeu-gonflable-spiderman', nom:'Spiderman', prix:120, groupe:'petits' },
    { id:'jeux-gonflable-pat-patrouille-2', nom:'Pat Patrouille', prix:140, groupe:'petits' },
    { id:'piscine-a-balle-xl', nom:'Piscine à Balle XL', prix:150.0, groupe:'petits' },
    { id:'jeu-gonflable-minions', nom:'Minions', prix:150, groupe:'xl' },
    { id:'jeu-gonflable-pat-patrouille-xl', nom:'Pat Patrouille XL', prix:150, groupe:'xl' },
    { id:'jeux-gonflable-avengers', nom:'Avengers', prix:180, groupe:'xl' },
    { id:'jeux-gonflable-parcours-dobstacle', nom:'Parcours d’Obstacle', prix:160, groupe:'xl' },
    { id:'jeux-gonflable-reine-des-neiges-co', nom:'Reine des Neiges XL', prix:180, groupe:'xl' },
    { id:'location-jeu-gonflable-princesse-x', nom:'Princesse XL', prix:180, groupe:'xl' },
    { id:'location-jeu-gonflable-licorne', nom:'Licorne', prix:200, groupe:'xl' },
    { id:'jeu-gonflable-spiderman-xl', nom:'Spiderman XL', prix:200, groupe:'xl' },
    { id:'location-jeu-gonflable-mario-bros', nom:'Mario Bros', prix:200, groupe:'xl' },
    { id:'location-jeu-gonflable-mickey-mous', nom:'Mickey Mouse', prix:280, groupe:'xl' },
    { id:'jeu-gonflable-palmiers', nom:'Jeu Gonflable Palmiers', prix:180.0, groupe:'xl' },
    { id:'jeu-gonflable-reine-des-neiges', nom:'Jeu Gonflable Reine des Neiges', prix:200.0, groupe:'xl' },
    { id:'jeux-gonflable-chateau-du-prince', nom:'Jeux Gonflable Château du Prince', prix:100.0, groupe:'xl' },
    { id:'jeux-gonflable-chateau-multicolore', nom:'Jeux Gonflable Château Multicolore', prix:120.0, groupe:'xl' },
    { id:'jeux-gonflable-flash-mcqueen', nom:'Jeux Gonflable Flash McQueen', prix:100.0, groupe:'xl' },
    { id:'sucture-gonflable-bunker', nom:'Sucture Gonflable (Bunker)', prix:25.0, groupe:'xl' },
    { id:'soccer-dart-gonflable-geant', nom:'Soccer Dart Gonflable Géant', prix:60, groupe:'sport' },
    { id:'jeux-gonflable-terrain-de-basket', nom:'Terrain de Basket', prix:100, groupe:'sport' },
    { id:'dunk-tank', nom:'Dunk Tank', prix:100.0, groupe:'sport' },
    { id:'laser-tag', nom:'Laser Tag', prix:120.0, groupe:'sport' },
    { id:'pouf-poire-gonflable', nom:'Pouf Poire Gonflable', prix:15, groupe:'extra' },
    { id:'divan-gonflable', nom:'Divan Gonflable', prix:20, groupe:'extra' },
    { id:'ecran-gonflable-20-pieds', nom:'Écran Gonflable 20 pieds', prix:60, groupe:'extra' },
    { id:'ecran-gonflable-16-pied', nom:'Écran Gonflable 16 pied', prix:50.0, groupe:'extra' }
  ];

  var $ = function(id){ return document.getElementById(id); };
  var elInv = $('jgInv'), elCP = $('jgCP'), elVille = $('jgVille');
  if (!elInv) return;
  ...
  function render(){
    $('jgCart').classList.remove('jg-show-offre');
    ...
    var ul = $('jgPanier');
    if (!lignes.length) {
      ul.innerHTML = '<li class="jg-vide"><span>Choisis au moins un article</span><span></span></li>';
    } else {
      ul.innerHTML = lignes.map(function(l){
        return '<li><span>' + l.q + ' × ' + l.nom + '</span><span>' + money(l.montant) + '</span></li>';
      }).join('');
    }
    $('jgSousTotal').textContent = lignes.length ? money(sousTotal) : '-';
    $('jgCta').disabled = !lignes.length;
    ...
    if (zone === null) { elVille.textContent = 'En attente'; elVille.classList.add('jg-inconnu'); }
    else if (zone === 'UNKNOWN') { elVille.textContent = 'Secteur à confirmer'; elVille.classList.add('jg-inconnu'); }
    else { elVille.textContent = zone[0]; elVille.classList.remove('jg-inconnu'); }

    var ligneLiv = $('jgLigneLiv');
    ligneLiv.classList.remove('jg-free');

    if (!lignes.length) {
      $('jgLiv').textContent = '-';
      $('jgTotal').textContent = '-';
      $('jgNote').textContent = 'Sélectionne tes articles pour voir ton prix. Prix avant taxes.';
      return;
    }

    if (etat.mode === 'ramassage') {
      ligneLiv.classList.add('jg-free');
      $('jgLiv').textContent = 'Gratuit';
      $('jgTotal').textContent = money(sousTotal);
      $('jgNote').textContent = 'Ramassage à Sainte-Thérèse, sans frais. Prévois un véhicule assez grand. Prix avant taxes.';
      return;
    }
    ...
    if (zone === null) {
      $('jgLiv').textContent = '-';
      $('jgTotal').textContent = money(sousTotal) + ' +';
      $('jgNote').textContent = 'Entre ton code postal pour ajouter la livraison au total. Prix avant taxes.';
      return;
    }
    ...
    if (liv === null) {
      etat.surDevis = true;
      $('jgLiv').textContent = 'Sur devis';
      $('jgTotal').textContent = money(sousTotal) + ' +';
      $('jgNote').textContent = (zone === 'UNKNOWN')
    ...
    etat.livraison = liv;
    $('jgLiv').textContent = money(liv);
    $('jgTotal').textContent = money(sousTotal + liv);
    $('jgNote').textContent = 'Livraison, installation et reprise comprises. Prix avant taxes.';
  }
  ...
  function afficherOffre(){
    ...
    var eyebrow = $('jgOffreEyebrow'), titre = $('jgOffreTitre'), texte = $('jgOffreTexte');
    var liste = $('jgOffreListe'), cta = $('jgOffreCta');
    ...
    if (etat.mode === 'livraison' && !etat.cpConnu) {
      ...
      $('jgCart').classList.add('jg-show-offre');
      elCP.focus();
      return;
    }
  ...
  function resume(){
    var t = [];
    t.push('Configurateur jeux gonflables'.toUpperCase());
    t.push('');
    t.push('Combien d\'enfants : ' + etat.invites);
    ...
    t.push('Livraison : ' + $('jgLiv').textContent);
    t.push('TOTAL ESTIME : ' + $('jgTotal').textContent);
    return t.join(LN);
  }
  ...
  function remplirRecap(){
    ...
    h += '<br>Livraison , <b>' + $('jgLiv').textContent + '</b>';
    h += '<br>Total estim&eacute; , <b>' + $('jgTotal').textContent + '</b>';
  ...
  function msg(txt){
    var m = $('jgMsg');
    m.textContent = txt;
    m.className = 'jg-msg jg-on jg-fail';
  }

  function ouvrirFormulaire(){
    if (!lignesPanier().length) return;
    remplirRecap();
    $('jgMsg').className = 'jg-msg';
    $('jgCart').classList.remove('jg-show-offre');
    $('jgCart').classList.add('jg-show-form');
    $('jgNom').focus();
  }

  function courrielValide(v){ return /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v); }

  $('jgCta').addEventListener('click', ouvrirFormulaire);
  $('jgOffreCta').addEventListener('click', function(e){
    if ((this.getAttribute('href') || '').indexOf('tel:') === 0) return;
    e.preventDefault();
    ouvrirFormulaire();
  });
  $('jgFormBack').addEventListener('click', function(){ $('jgCart').classList.remove('jg-show-form'); });
  $('jgNo').addEventListener('click', afficherOffre);
  $('jgOffreBack').addEventListener('click', function(){ $('jgCart').classList.remove('jg-show-offre'); });

  $('jgForm').addEventListener('submit', function(e){
    e.preventDefault();
    var nom = $('jgNom').value.trim(), email = $('jgEmail').value.trim();
    $('jgNom').parentElement.classList.toggle('jg-err', !nom);
    $('jgEmail').parentElement.classList.toggle('jg-err', !courrielValide(email));
    ...
    fd.append('telephone', $('jgTel').value.trim());
    fd.append('date_event', $('jgDate').value);
    fd.append('details', details);
    fd.append('service', 'Configurateur jeux gonflables');
  ...
  function majQte(id, q){
    panier[id] = q;
    var ligne = document.querySelector('.jg-page .jg-item[data-id="' + id + '"]');
    if (ligne) {
      ligne.querySelector('.jg-qval').textContent = q;
      ligne.classList.toggle('jg-actif', q > 0);
    }
    if (visioId === id) $('jgVisioQte').textContent = q;
    render();
  }
  ...
  /* ---------- Visionneuse produit ---------- */
  var visio = $('jgVisio'), visioId = null;
  ...
  function ouvrirVisio(id){
    ...
    var note = ligne ? ligne.querySelector('.jg-item-note') : null;
    /* Tous les produits Booqable n'ont pas de photo. Ouvrir une
       visionneuse vide serait pire que ne rien ouvrir. */
    if (!img) return;
    visioId = id;
    $('jgVisioImg').setAttribute('src', img ? img.getAttribute('src') : '');
    $('jgVisioImg').setAttribute('alt', p.nom);
    $('jgVisioNom').textContent = p.nom;
    $('jgVisioNote').textContent = note ? note.textContent : '';
    $('jgVisioPrix').textContent = p.prix + ' $';
    $('jgVisioQte').textContent = panier[id] || 0;
    visio.hidden = false;
  ...
  document.querySelectorAll('.jg-page .jg-item-img').forEach(function(v){
    v.addEventListener('click', function(){
      var ligne = this.closest('.jg-item');
      if (ligne) ouvrirVisio(ligne.getAttribute('data-id'));
    });
  });
  $('jgVisioX').addEventListener('click', fermerVisio);
  $('jgVisioFond').addEventListener('click', fermerVisio);
  $('jgVisioMoins').addEventListener('click', function(){ if (visioId) changerQte(visioId, -1); });
  $('jgVisioPlus').addEventListener('click', function(){ if (visioId) changerQte(visioId, 1); });
  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape' && !visio.hidden) fermerVisio();
  });

  /* ---------- Entrees ---------- */
  document.querySelectorAll('.jg-page .jg-item .jg-qbtn').forEach(function(b){
  ...
  document.querySelectorAll('.jg-page .jg-mode').forEach(function(b){
    b.addEventListener('click', function(){
      etat.mode = this.getAttribute('data-mode');
      document.querySelectorAll('.jg-page .jg-mode').forEach(function(x){ x.classList.remove('jg-on'); });
      this.classList.add('jg-on');
      $('jgEtapeCP').classList.toggle('jg-cache', etat.mode === 'ramassage');
      render();
    });
  });

  $('jgMoins').addEventListener('click', function(){
    elInv.value = Math.max(1, (parseInt(elInv.value, 10) || 1) - 5); render();
  });
  $('jgPlus').addEventListener('click', function(){
  ...
  /* FAQ */
  document.querySelectorAll('.jg-page .faq-question').forEach(function(btn){
    btn.addEventListener('click', function(){
      var wasOpen = this.classList.contains('open');
      document.querySelectorAll('.jg-page .faq-question.open').forEach(function(b){
  ...
  try {
    var faqLd = [];
    document.querySelectorAll('.jg-page .faq-question').forEach(function(b){
