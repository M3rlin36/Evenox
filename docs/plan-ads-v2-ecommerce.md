...
- [ ] **Moyen de paiement valide et garde-fous de dépense.** Meta → *Facturation* → *Limite de dépense du compte* = **1 000 $**. Attention : cette limite est CUMULATIVE, pas mensuelle. Elle ne se remet pas à zéro toute seule, et quand elle est atteinte, toutes les pubs s'arrêtent d'un coup. Rappel d'agenda au 26 octobre pour la relever à 2 000 $ (même valeur qu'en section 4). Côté Google, il n'existe pas de plafond mensuel de compte en paiement automatique : les garde-fous sont le budget quotidien de 16,50 $ et la date de fin de campagne au 22 novembre. Si ton compte est en facturation mensuelle sur facture, le champ *Budget du compte* existe : mets-y 600 $. *10 min.* **Fait quand** : la limite Meta est enregistrée, le rappel du 26 octobre est dans l'agenda, et la date de fin de la campagne Google est au 22 novembre.
...
### 6.2 Page des lettres : testée le 19 septembre, le sélecteur fonctionne

**Résultat du test, fait en navigateur réel sur un affichage iPhone de 390 pixels.** Le sélecteur de lettres s'affiche bien : une liste déroulante de 28 entrées, soit l'invite « Sélectionner une variante » et les 27 caractères de A à Z plus l'esperluette. Le choix d'une lettre est pris en compte, le prix affiche « 2 Jours 70,00 $ », le champ de quantité est là, et le bouton « Ajouter au panier » ouvre bien la suite du parcours : choix entre cueillette et livraison, adresse de l'entrepôt affichée, puis bouton « Passer aux dates » qui ouvre un calendrier où les dates passées sont grisées. La crainte principale du plan est donc levée : **il n'y a pas de repli à construire, la grille de 27 boutons n'est pas nécessaire.**
...
**Ancien plan de repli, conservé au cas où le test manuel échouerait.** **Deux replis.** A : une **grille de 27 boutons** dans la page Divi, un `bq-product-button` par variante (A à Z plus `&`), chacun pointant son identifiant. B : envoyer le trafic à la fiche `marquee-letter` sur `evenox.booqableshop.com`, où le sélecteur natif fonctionne.
